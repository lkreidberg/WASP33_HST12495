...
for i in enumerate(files):
	...
	if(ima[0].header['filter'] == obs_par['FILTER']):
		...
		if output==True: 
			print>>f, t, results[3]+rmin-LTV1, results[2]+cmin-LTV2 -112 
			if np.abs(t - t_last) > 5./24.:							#checks if this image corresponds to a new visit
				print>>tstart, np.floor(t)	
				t_last = t
		
		print t, results[3]+rmin-LTV1, results[2]+cmin-LTV2 - 112, files[i[0]] 		#fit results

#NOTE: I'm subtracting 112 from spectral direction due to difference in CRPIX1 value in header for grism vs direct image
# the difference was 122, but 112 gives a much more correct wavelength solution (compared to grism sensitivity).
# I also used 112 for the HD 97658 data.  Maybe there's an extra 10 pix because of the reference pixels??
