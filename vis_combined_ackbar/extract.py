...
for f in ancil.files:
    ...
    for ii in range(d[0].header['nsamp']-2):    
        ...
        [skyrmin, skyrmax] = [9, 16]
        [skycmin, skycmax] = [246, 258]
    ...
    """plt.plot(template_waves, spec_opt/np.max(spec_opt))
    s = np.genfromtxt("config/WFC3_G141_sensitivity.txt")
    star = blackbody(s[:,0]/1.e6, 7400)
    s[:,1] *= star
    plt.plot(s[:,0]*1e4, s[:,1]/np.max(s[:,1]), color='r')
    plt.show()"""
